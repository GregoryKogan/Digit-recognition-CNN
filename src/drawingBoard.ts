...
export class DrawingBoard {
    ...
    for (const point of points) {
      ...
      for (let io = -offset; io <= offset; io++) {
        for (let jo = -offset; jo <= offset; jo++) {
          ...
          if (
            ti < 0 ||
            ti >= this.widthInPixels ||
            tj < 0 ||
            tj >= this.widthInPixels
          )
            continue;
          const pixelCenterX =
            this.startX + tj * this.pixelWidth - this.pixelWidth / 2;
          const pixelCenterY =
            this.startY + ti * this.pixelWidth - this.pixelWidth / 2;
          const distToMouse = sketch.dist(
            point.x,
            point.y,
            pixelCenterX,
            pixelCenterY
          ...
          this.data[linearInd] += sketch.map(
            distToMouse,
            0,
            this.brushRadius,
            1,
          ...
          if (this.data[linearInd] != oldValue) this.renderPixel(linearInd);
        }
      }
    }
  }
  ...
  fullRender() {
    ...
    for (let ind = 0; ind < this.data.length; ++ind) this.renderPixel(ind);
  }
